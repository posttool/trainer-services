...
public class HTSPStream {
    ...
    // private final HTSDWin dw; /* Windows used to calculate dynamic features,
    ...
    public HTSPStream(int vector_size, int utt_length, HMMData.FeatureType fea_type, int maxIterationsGV)
         ...
         */
        /*
         * - InitDwin reads the window files passed as parameters for example:
         * mcp.win1, mcp.win2, mcp.win3
         */
        /*
         * for the moment the dynamic window is the same for all MCP, LF0, STR
         * and MAG
         */
        /*
    ...
    public void mlpg(HMMData htsData, boolean useGV) {
        ...
        if (htsData.getUseContextDependentGV())
            ...
            System.out.println("Global variance optimization");
        ...
        for (int m = 0; m < order; m++) {
            ...
            forwardSubstitution(mywum, mywuw); /*
                                                * forward substitution in
                                                * Cholesky decomposition
                                                */
            backwardSubstitution(m, mywuw); /*
                                             * backward substitution in Cholesky
                                             * decomposition
                                             */
    ...
    private void calcWUWandWUM(int m) {
         ...
         * k<wuw[t].length; k++) System.out.format("%f ", wuw[t][k]);
         * System.out.format("\n"); } System.out.format("\n"); }
         */
    }
    ...
    private static void ldlFactorization(double[][] mywuw) {
        for (int t = 0; t < mywuw.length; t++) {
            ...
            /*
             * I need i=1 for the delay in t, but the indexes i in WUW[t][i] go
             * from 0 to 2 so wherever i is used as index i=i-1 (this is just to
             * keep somehow the original c implementation).
             */
            ...
            /*
             * if(debug) { System.out.println("LDL factorization:");
             * printWUW(t); System.out.println(); }
             */
        }
    ...
    private void forwardSubstitution(double[] mywum, double[][] mywuw) {
        ...
        /*
         * for(t=0; t<nT; t++) System.out.format("%f ", g[t]);
         * System.out.println();
         */
    }
    ...
    private void gvParmGenDerivative(int m, boolean debug) {
        ...
        /*
         * first convert c (c=par) according to GV pdf and use it as the initial
         * value
         */
        convGV(m);
        ...
        for (iter = 1; iter <= maxGVIter; iter++) {
            ...
            /*
             * steepest ascent and quasy Newton c(i+1) = c(i) + alpha *
             * grad(c(i))
             */
            for (t = 0; t < nT; t++)
                par[t][m] += step * g[t];
        ...
        System.out.println("Derivative GV optimization for feature: (" + m + ")  number of iterations=" + (iter - 1));
    ...
    private void gvParmGenGradient(int m, boolean debug) {
        ...
        for (iter = 1; iter <= maxGVIter; iter++) {
            ...
            if (iter > 1) {
                ...
                if (obj > prev) {
                    step *= stepInc;
                    // logger.info("+++ obj > prev iter=" + iter +"  obj=" + obj
                    // + "  > prev=" + prev);
                    numDown = 0;
                }
                /*
                 * objective function degraded -> go back c and decrese step
                 * size
                 */
                if (obj < prev) {
                    for (t = 0; t < nT; t++)
                    ...
                    numDown++;
                    // logger.info("--- obj < prev iter=" + iter +"  obj=" + obj
                    // + "  < prev=" + prev +"  numDown=" + numDown);
                    if (numDown < 100)
                        continue;
                    else {
                        System.out.println("  ***Convergence problems....optimization stopped. Number of iterations: "
                                + iter);
                        break;
                    }
                }
                ...
                if (debug)
                    System.out.println("  First iteration:  GVobj=" + obj + " (HMMobj=" + HMMobj + "  GVobj=" + GVobj
                            + ")");
            }
            /* convergence check (Euclid norm, objective function) */
            if (norm < minEucNorm || (iter > 1 && Math.abs(obj - prev) < GVepsilon)) {
                if (debug)
                    System.out.println("  Number of iterations: [   " + iter + "   ] GVobj=" + obj + " (HMMobj="
                            + HMMobj + "  GVobj=" + GVobj + ")");
                ...
                if (debug) {
                    if (iter > 1)
                        System.out.println("  Converged (norm=" + norm + ", change=" + Math.abs(obj - prev) + ")");
                    else
                        System.out.println("  Converged (norm=" + norm + ")");
                }
                break;
            }
            /*
        ...
        if (iter > maxGVIter) {
            System.out
                    .println("   optimization stopped by reaching max number of iterations (no global variance applied)");

            /*
             * If there it does not converge, the feature parameter is not
             * optimized
             */
            for (t = 0; t < nT; t++) {
                par[t][m] = par_ori[t];
        ...
        System.out.println("Gradient GV optimization for feature: (" + m + ")  number of iterations=" + totalNumIter);
    }
    ...
    private double calcGradient(int m) {
         ...
         * in Toda and Tokuda IEICE-2007 paper.
         */
        ...
        for (t = 0; t < nT; t++) {
            ...
            for (i = 2; i <= WIDTH; i++) { /* WIDTH goes from 0 to 2 WIDTH=3 */
                if (t + i - 1 < nT)
                    g[t] += wuw[t][i - 1] * par[t + i - 1][m]; /*
                                                                * i as index
                                                                * should be i-1
                                                                */
                if (t - i + 1 >= 0)
                    g[t] += wuw[t - i + 1][i - 1] * par[t - i + 1][m]; /*
                                                                        * i as
                                                                        * index
                                                                        * should
                                                                        * be i-1
        ...
        norm = Math.sqrt(norm);
        // logger.info("HMMobj=" + HMMobj + "  GVobj=" + GVobj + "  norm=" +
        // norm);
    ...
    private double calcDerivative(int m) {
        ...
        for (t = 0, HMMobj = 0.0; t < nT; t++) {
            ...
            h = -w1
                    ...
                    * 2.0
                    / (nT * nT)
                    * ((nT - 1) * gvcovInv[m] * (var - gvmean[m]) + 2.0 * gvcovInv[m] * (par[t][m] - mean)
                            * (par[t][m] - mean));
    ...
    private void convGV(int m) {
        ...
        /*
         * c'[t][d] = ratio * (c[t][d]-mean[d]) + mean[d] eq. (34) in Toda and
         * Tokuda IEICE-2007 paper.
         */
        for (t = 0; t < nT; t++) {
            if (gvSwitch[t])
