...
public class HMMVoiceConfigure {
    ...
    public SortedMap<String, String> getDefaultProps() {
        ...
        // Frame period in point 80 for 16Khz; 240 for 48Khz (Frame period
        // in point = sampRate*0.005sec)");
        int sampRate = getSamplingRate();
        int frameLen = (int) Math.round(sampRate * 0.025);
        int frameShift = (int) Math.round(sampRate * 0.005);
        props.put(SAMPFREQ, getProp(SAMPLINGRATE));
    ...
    protected void setupHelp() {
        ...
        props2Help.put(MGCBANDWIDTH,
                "band width for MGC transforms (default=24 for cepstral form, derault=1 for LSP form)");
    ...
    public boolean compute() throws Exception {
        ...
        String filedir = getProp(ROOTDIR);
        String cmdLine;
        ...
        if (getProp(ADAPTSCRIPTS).contentEquals("false")) {
            ...
            if (!dirWav.exists() || dirWav.list().length == 0 || !dirRaw.exists() || dirRaw.list().length == 0) {
                System.out
                        .println("Problem with wav and hts/data/raw directories: wav files and raw files do not exist"
            ...
            // Check if phonefeatures and phonelab directory exist and have
            // files
            if ((!dirFea.exists() || dirFea.list().length == 0) && (!dirLab.exists() || dirLab.list().length == 0)) {
                System.out
                        .println("Problems with directories phonefeatures or phonelab, they do not exist or they are empty.");
                speech_transcriptions = false;
            }
        ...
        if (speech_transcriptions) {
            ...
            if (getProp(ADAPTSCRIPTS).contentEquals("false")) {
                ...
                /*
                 * first it should go to the hts directory and there run
                 * ./configure
                 */
                ...
                cmdLine = "cd " + filedir + "hts\n" + getProp(CONFIGUREFILE) + " --with-tcl-search-path="
                        + getExternal(TCLPATH) + " --with-sptk-search-path="
                        + getExternal(SPTKPATH) + " --with-hts-search-path="
                        + getExternal(HTSPATH) + " --with-hts-engine-search-path="
                        + getExternal(HTSENGINEPATH) + " --with-sox-search-path="
                        + getExternal(SOXPATH) + " SPEAKER=" + getProp(SPEAKER) + " DATASET="
                        ...
                        + getProp(FRAMESHIFT) + " WINDOWTYPE=" + getProp(WINDOWTYPE) + " NORMALIZE="
                        + getProp(NORMALIZE) + " FFTLEN=" + getProp(FFTLEN) + " FREQWARP=" + getProp(FREQWARP)
                        + " GAMMA=" + getProp(GAMMA) + " MGCORDER=" + getProp(MGCORDER) + " STRORDER="
                        + getProp(STRORDER) + " STRFILTERNAME=" + getProp(STRFILTERNAME) + " LNGAIN=" + getProp(LNGAIN)
                        + " SAMPFREQ=" + getProp(SAMPFREQ) + " NSTATE=" + getProp(NSTATE) + " NITER=" + getProp(NITER);
                ...
                cmdLine = "cd " + filedir + "hts\n" + getProp(CONFIGUREFILE) + " --with-tcl-search-path="
                        ...
                        + getExternal(HTSENGINEPATH) + " --with-sox-search-path="
                        + getExternal(SOXPATH) + " SPEAKER=" + getProp(SPEAKER) + " DATASET="
                        + getProp(DATASET) + " TRAINSPKR=" + getProp(ADAPTTRAINSPKR) + " ADAPTSPKR="
                        + getProp(ADAPTSPKR) + " F0_RANGES=" + getProp(ADAPTF0_RANGES) + " SPKRMASK="
                        + getProp(ADAPTSPKRMASK) + " ADAPTHEAD=" + getProp(ADAPTHEAD) + " VER=" + getProp(VER)
                        + " QNUM=" + getProp(QNUM) + " FRAMELEN=" + getProp(FRAMELEN) + " FRAMESHIFT="
                        + getProp(FRAMESHIFT) + " WINDOWTYPE=" + getProp(WINDOWTYPE) + " NORMALIZE="
                        + getProp(NORMALIZE) + " FFTLEN=" + getProp(FFTLEN) + " FREQWARP=" + getProp(FREQWARP)
                        + " GAMMA=" + getProp(GAMMA) + " MGCORDER=" + getProp(MGCORDER) + " STRORDER="
                        + getProp(STRORDER) + " STRFILTERNAME=" + getProp(STRFILTERNAME) + " LNGAIN=" + getProp(LNGAIN)
                        + " SAMPFREQ=" + getProp(SAMPFREQ) + " NSTATE=" + getProp(NSTATE) + " NITER=" + getProp(NITER)
                        + " MGCBANDWIDTH=" + getProp(MGCBANDWIDTH) + " STRBANDWIDTH=" + getProp(STRBANDWIDTH)
                        + " LF0BANDWIDTH=" + getProp(LF0BANDWIDTH) + " TREEKIND=" + getProp(ADAPTTREEKIND)
            ...
            System.out
                    .println("Problems with directories phonefeatures or phonelab, they do not exist or they are empty.");
            System.out
                    .println("Problems with directories wav, text or hts/data/raw, they do not exist or they are empty.");
            return false;
        }
