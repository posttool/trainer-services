...
public class MathUtils {
     ...
     *            <p>
     *            1: normalizes with N, this provides the square root of the
    ...
    public static ComplexNumber divideComplex(ComplexNumber x1, ComplexNumber x2) {
        ...
        return new ComplexNumber((x1.real * x2.real + x1.imag * x2.imag) / denum, (x1.imag * x2.real - x1.real
                * x2.imag)
                / denum);
    }
    ...
    // for (int i = startInd; i <= endInd; i++)
    // absVals[i - startInd] = Math.sqrt(c.real[i] * c.real[i] + c.imag[i] *
    // c.imag[i]);
    //
    // return absVals;
    // }
    ...
    // public static double[] dft2ampdb(ComplexArray c, int startInd, int
    // endInd) {
    // if (startInd < 0)
    // startInd = 0;
    // if (startInd > Math.min(c.real.length - 1, c.imag.length - 1))
    ...
    // for (int i = startInd; i <= endInd; i++)
    // dbs[i - startInd] = amp2db(Math.sqrt(c.real[i] * c.real[i] + c.imag[i] *
    // c.imag[i]));
    //
    // return dbs;
    // }
            ...
            if (N == 1) {
                ...
                for (int i = 0; i < newLength; i++) {
                    ...
                    if (leftInd < x.length - 1) {
                        ...
                        if (leftInd > 0) {
                            yReal = interpolatedSample(leftInd, i * ratio, leftInd + 1, x[leftInd].real, 2
                                    * x[leftInd].real - x[leftInd - 1].real);
                            yImag = interpolatedSample(leftInd, i * ratio, leftInd + 1, x[leftInd].imag, 2
                                    * x[leftInd].imag - x[leftInd - 1].imag);
                            y[i] = new ComplexNumber(yReal, yImag);
                        } else
    ...
    // public static int[] findOr(int[] x, int comparator1, int val1, int
    // comparator2, int val2) {
    // double[] xd = new double[x.length];
    // for (int i = 0; i < x.length; i++)
    // xd[i] = (double) x[i];
    //
    ...
    // val2
    // public static int[] findOr(double[] x, int comparator1, double val1, int
    // comparator2, double val2) {
    // int[] indices = null;
    // int[] indices1 = find(x, comparator1, val1);
    // int[] indices2 = find(x, comparator2, val2);
    //
    ...
    /***
     * Calcualtes x_i = (x_i - mean(x)) / std(x) This function can deal with
     * NaNs
     * 
     * @param x
     * @return
    ...
    // else
    // y[i] = String.valueOf(x.real[i]) + "-i*" +
    // String.valueOf(Math.abs(x.imag[i]));
    // }
    // }
    //
    // return y;
