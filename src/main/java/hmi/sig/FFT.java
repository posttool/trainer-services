...
public class FFT {
     ...
     * @return the power spectrum, as an array of length N/2 (where N is the
     *         power of two greater than or equal to signal.length): the log of
     ...
     *         the log of the square of the absolute value of each complex
     ...
     *            the real signal for which to compute the power spectrum.
     * @return the power spectrum, as an array of length N/2 (where N is the
     *         power of two greater than or equal to signal.length): the squared
     *         absolute values of the lower half of the complex fourier
     *         transform array.
     */
    ...
    /**
     * Convenience method for computing the absolute amplitude spectrum of a
     * real signal. The signal can be of any length; internally, zeroes will be
     * added if signal length is not a power of two.
     * 
     * @param signal
     *            the real signal for which to compute the power spectrum.
     * @return the power spectrum, as an array of length N/2 (where N is the
     *         power of two greater than or equal to signal.length): the
     *         absolute values of the lower half of the complex fourier
     *         transform array.
     */
    ...
    /**
     * From the result of the FFT (in the frequency domain), compute the
     * absolute value for each positive frequency, i.e. the norm of each complex
     * number in the lower half of the array
     * 
     * @param fft
     *            the array of real and imag parts of the complex number array,
     *            fft[0] = real[0], fft[1] = real[N/2], fft[2*i] = real[i],
    ...
    public static double[] computePhaseSpectrum_FD(final double[] fft) {
        ...
        phases[0] = Math.atan2(0, fft[0]); // and ignore fft[1], which is
                                           // actually real[halfN].
        for (int i = 2; i < fft.length; i += 2) {
    ...
    /**
     * Carry out the FFT or inverse FFT, and return the result in the same
     * arrays given as parameters. In the case of the "forward" FFT, real is the
     * signal to transform, and imag is an empty array. After the call, real
     * will hold the real part of the complex frequency array, and imag will
     * hold the imaginary part. They are ordered such that first come positive
     * frequencies from 0 to fmax, then the negative frequencies from -fmax to 0
     * (which are the mirror image of the positive frequencies). In the case of
     * the inverse FFT, real and imag are in input the real and imaginary part
     * of the complex frequencies, and in output, real is the signal. The method
     * already computes the division by array length required for the inverse
     * transform.
     * 
     * @param real
     *            in "forward" FFT: as input=the time-domain signal to
     *            transform, as output=the real part of the complex frequencies;
     *            in inverse FFT: as input=the real part of the complex
     *            frequencies, as output= the time-domain signal.
     * @param imag
     *            in "forward" FFT: as input=an empty array, as output=the
     *            imaginary part of the complex frequencies; in inverse FFT: as
     *            input=the imaginary part of the complex frequencies, as
     *            output= not used.
     * @param inverse
    ...
    public static void transform(double[] real, double[] imag, boolean inverse) {
        ...
        for (int i = 0; i < N; i++) {
            if (i > iReverse) {
                // System.err.println("Swapping " + Integer.toBinaryString(i) +
                // " with " + Integer.toBinaryString(iReverse));
    ...
    /**
     * Carry out the FFT or inverse FFT, and return the result in the same
     * arrays given as parameters. This works exactly like #transform(real,
     * imag, boolean), but data is represented differently: the even indices of
     * the input array hold the real part, the odd indices the imag part of each
     * complex number.
     * 
     * @param realAndImag
    ...
    public static void realTransform(double data[], boolean inverse) {
        ...
        for (int i = 1; i < n4; i++) { // Case i=0 done separately below.
            ...
            double h1r = c1 * (data[twoI] + data[nMinusTwoI]); // The two
                                                               // separate
                                                               // transforms are
                                                               // separated out
                                                               // of data.
    ...
    /**
     * Compute the convolution of two signals, by multipying them in the
     * frequency domain. Normalise the result with respect to deltaT (the
     * inverse of the sampling rate). This method applies zero padding where
     * necessary to ensure that the result is not polluted because of assumed
    ...
    /**
     * Compute the convolution of two signals, by multipying them in the
     * frequency domain. This method applies zero padding where necessary to
     * ensure that the result is not polluted because of assumed periodicity.
     * The two signals need not be of equal length.
     * 
     * @param signal1
     * @param signal2
     * @return the convolved signal, with length signal1.length+signal2.length
     ...
     * frequency domain. Normalise the result with respect to deltaT (the
     * inverse of the sampling rate). This is the core method, requiring two
     * signals of equal length, which must be a power of two, and not checking
     ...
     * Compute the convolution of two signals, by multiplying them in the
     * frequency domain. Normalise the result with respect to deltaT (the
     * inverse of the sampling rate). This is a specialised version of the core
     * method, requiring two signals of equal length, which must be a power of
     * two, and not checking for pollution arising from the assumed periodicity
     * of both signals. In this version, the first signal is provided in the
     * time domain, while the second is already transformed into the frequency
     * domain.
     * 
     * @param signal1
     ...
     *            domain fft[0] = real[0], fft[1] = real[N/2], fft[2*i] =
     ...
     * signals. In this version, the first signal is provided in the time
     * domain, while the second is already transformed into the frequency
     * domain.
     * 
     * @param signal1
     *            the first input signal, in the time domain
    ...
    /**
     * Compute the correlation of two signals, by multipying them in the
     * frequency domain. This method applies zero padding where necessary to
     * ensure that the result is not polluted because of assumed periodicity.
     * The two signals need not be of equal length.
     * 
     * @param signal1
     * @param signal2
     * @return the correlation function, with length
     ...
     * frequency domain. Sign convention: If signal2 is shifted by n to the
     * right of signal2, then the correlation function will have a peak at
     * positive n. This is the core method, requiring two signals of equal
     * length, which must be a power of two, and not checking for pollution
     * arising from the assumed periodicity of both signals.
     * 
     * @param signal1
     * @param signal2
     * @return the correlated signal, of the same length as the two input
     *         signals
     * @throws IllegalArgumentException
     *             if the two input signals do not have the same length.
     */
    ...
    /**
     * Compute the autocorrelation of a signal, by inverse transformation of its
     * power spectrum. This method applies zero padding where necessary to
     * ensure that the result is not polluted because of assumed periodicity.
     * 
     * @param signal
     * @return the correlated signal, of the same length as the input signal
