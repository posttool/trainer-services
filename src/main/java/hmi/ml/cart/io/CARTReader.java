...
public class CARTReader {
	...
	public CART load(String fileName) throws IOException, Exception {
	...
	public CART loadFromStream(InputStream inStream) throws IOException, Exception {
		// open the CART-File and read the header
		DataInput raf = new DataInputStream(new BufferedInputStream(inStream));

		CARTHeader header = new CARTHeader(raf);
		if (!header.hasCurrentVersion()) {
			throw new IOException("Wrong version of database file");
		}
		if (header.getType() != CARTHeader.CARTS) {
			...
			case ByteDecisionNode:
				...
				if (featureDefinition.getNumberOfValues(featureNameIndex) != numChildren) {
					throw new IOException("Inconsistent cart file: feature "
							...
							+ featureDefinition.getNumberOfValues(featureNameIndex) + " values, but decision node " + i
							+ " has only " + numChildren + " child nodes");
				}
		...
		// Now, link up the decision nodes with their children
		for (int i = 0; i < numDecNodes; i++) {
			for (int k = 0; k < childIndexes[i].length; k++) {
				...
				if (childIndex < 0) { // a decision node
					...
					dns[i].addChild(dns[-childIndex - 1]);
				} else if (childIndex > 0) { // a leaf node
					dns[i].addChild(lns[childIndex - 1]);
	...
	private CART loadFromByteBuffer(String fileName) throws IOException, Exception {
		// open the CART-File and read the header
		FileInputStream fis = new FileInputStream(fileName);
		...
		CARTHeader header = new CARTHeader(bb);
		if (!header.hasCurrentVersion()) {
			throw new IOException("Wrong version of database file");
		}
		if (header.getType() != CARTHeader.CARTS) {
